                ...
                PNGProps     *metadata_props)
{
	...
	if (metadata_props->license) {
		tracker_sparql_builder_predicate(metadata,
		                                  "nie:license");
		tracker_sparql_builder_object_unvalidated(metadata,
		                            metadata_props->license);
	}

	if (metadata_props->creator) {
		gchar *uri;
		uri = tracker_sparql_escape_uri_printf("urn:contact:%s",
	...
	if (metadata_props->make || metadata_props->model) {
		gchar *equip_uri;

		equip_uri = tracker_sparql_escape_uri_printf(
		          "urn:equipment:%s:%s:",
		          metadata_props->make ? metadata_props->make : "",
		          metadata_props->model ? metadata_props->model : "");
		...
		if (metadata_props->make) {
			tracker_sparql_builder_predicate(preupdate,
			                                  "nfo:manufacturer");
			tracker_sparql_builder_object_unvalidated(preupdate,
			                    metadata_props->make);
		}
	...
	tracker_sparql_builder_object_string(metadata, "image/png");

	/*
	* Set DLNA profile based on image size, disregard color
	* requirements for now
	*/
	if (metadata_props->width <= 48 &&
	    metadata_props->height <= 48)
		dlna_profile = "PNG_SM_ICO";
	else if (metadata_props->width <= 120 &&
	           metadata_props->height <= 120)
		dlna_profile = "PNG_LRG_ICO";
	else if (metadata_props->width <= 160 &&
	           metadata_props->height <= 160)
		dlna_profile = "PNG_TN";
	else if (metadata_props->width <= 4096 &&
	           metadata_props->height <= 4096)
		dlna_profile = "PNG_LRG";
	else
		dlna_profile = "UNKNOWN";
...
tracker_extract_get_metadata(TrackerExtractInfo *info)
{
	...
	tracker_sparql_builder_object(builder, "nfo:Image");
	tracker_sparql_builder_object(builder, "nmm:Photo");

	/* Width and height are guaranteed to be present in the header.. */
	tracker_sparql_builder_predicate(builder, "nfo:width");
	tracker_sparql_builder_object_int64(builder, metadata_props.width);

	tracker_sparql_builder_predicate(builder, "nfo:height");
